...
#
# Extended for CSC 526, Compilers II, Dr. D. Parson, Spring 2012,
...
#
# Extended multiple times in Spring 2012 to include parallel function calls
# and other features.
# -----------------------------------------------------------------------------
...
from optimize5.CodeGenerator import CodeGenerator
from optimize5.TreeOptimizer import TreeOptimizer
sys.path.insert(0,"../..")
...
# WE CANNOT DO THIS WITH MULTITHREADING -- There are many returning
...
#           SIDE NOTE: TreeOptimizer may change it to this:
...
# incoming fp & arguments before it returns. It leaves its return value
# onto the data stack and returns.
...
def p_expression_alen(p):
    ...
    if not tIsArray(arraytype):
        terror(TypeError, "ERROR, Non-array type at length near line " \
            + str(__global_line_number__))
        arraytype = None
    else:
        compat = 'int'      # LENGTH is an int
...
def p_expression_funcall(p):
    "expression : exprfuncall"
    p[0] = p[1]

def p_expression_parallel_funcall(p):
    "expression : exprparallel"
    p[0] = p[1]

def p_expression_funcall_help(p):
    "exprfuncall : ID LPAREN arglist RPAREN"
...
def tParallelFuncalls(reduceop, reduceval, funlist):
    '''
    tParallelFuncalls performs type-compatibility checking on a group
    of parallel function calls. reduceop is the reduction operator,
    None for a collect construct. reduceval is the optional reduce
    initialization constant, None when not used. funlist is the mandatory
    collection of function calls, with a min length of 2.
    This function's return value is the type of the functions -- they must be
    identical, and must be compatible with any reduceop -- or None when types
    are incompatible, in which case tParallelFuncalls logs the error via terror.
    '''
    firsttype = funlist[0][1]
    result = firsttype
    if (tIsArray(firsttype) and reduceop != '+'):
        terror(TypeError,   \
            "ERROR, parallel reduction operator near line " \
                + str(__global_line_number__) + ", operator " \
                + str(reduceop) + " is incompatible with array type "    \
                + str(firsttype))
        result = None
    for f in funlist:
        if f[1] != firsttype:
            terror(TypeError,   \
                "ERROR, parallel function call near line " \
                    + str(__global_line_number__) + " to function " \
                    + f[2] + " has incompatible type " + str(f[1])      \
                    + ", should be " + str(firsttype))
            result = None
    if (reduceval != None and reduceval[1] != firsttype):
        terror(TypeError,   \
            "ERROR, parallel reduction initialization value near line " \
                + str(__global_line_number__) + ", value " \
                + reduceval[2] + " has incompatible type " + reduceval[1] \
                + ", should be " + firsttype)
        result = None
    return result

def parallelFuncCallDepthOK(outerfunc, innerfunc, stab, limit):
    '''
    parallelFuncCallDepthOK checks parallel function calls rooted in
    a call to outerfunc, to make sure that none of outerfunc or its nested
    functions access any static scope outside that created by outerfunc in
    its initial, parallel call. It logs an error to terror and returns
    false if it finds any out-of-scope variable or function references,
    otherwise returning true.
    '''
    isok = True
    for entry in stab.values():
        if (isinstance(entry, (tuple, list))):
            if entry[0] == 'symref':
                symname = entry[1]
                symdepth = entry[2]
                deftab = entry[3]
                defentry = deftab[symname]
                if defentry[0] == 'var' and symdepth > limit:
                    msg = "ERROR, function " + innerfunc
                    if (outerfunc != innerfunc):
                        msg += ", nested in function " + outerfunc
                    msg += " uses outer variable " + symname
                    msg += " outside the scope of the parallel function."
                    terror(NameError, msg)
                    isok = False
                elif defentry[0] == 'func' and symdepth > limit:
                    if not(symdepth == (limit+1) and symname == outerfunc):
                        msg = "ERROR, function " + innerfunc
                        if (outerfunc != innerfunc):
                            msg += ", nested in function " + outerfunc
                        msg += " uses outer function " + symname
                        msg += " outside the scope of the parallel function."
                        terror(NameError, msg)
                        isok = False
            elif entry[0] == 'func':    # nested function definition
                if not parallelFuncCallDepthOK(outerfunc, entry[1], \
                        entry[4], limit+1):
                    isok = False
    return isok

def p_exprparallel(p):
    'exprparallel : LBRACE LBRACE parallelOp parallelFuncs RBRACE RBRACE'
    # ('preduce'|'pcollect' type opOrNone initvalueOrNone (funcalls))
    ptype = tParallelFuncalls(p[3][1], p[3][2], p[4])
    if ptype:
        if p[3][0] == 'pcollect':
            ptype = ptype + '[]'
        for f in p[4]:
            compat, location, outlinks = getSYMtypeOffset(f[2], f[3])
            if compat != None and outlinks != None:
                # Otherwise there is a problem with the function.
                outerframe = -1 - outlinks
                fentry = scopestack[outerframe][f[2]]
                parallelFuncCallDepthOK(f[2], f[2], fentry[4], 0)
    p[0] = (p[3][0], ptype, p[3][1], p[3][2], p[4])

def p_parallelOpReduce(p):
    'parallelOp : REDUCE reduceop reduceinit'
    p[0] = ('preduce', p[2], p[3])

def p_parallelOpAggregate(p):
    'parallelOp : COLLECT'
    p[0] = ('pcollect', None, None)
...
def p_parallelFuncs(p):
    '''parallelFuncs : exprfuncall SEP exprfuncall SEP parallelMoreFuncs'''
    p[0] = (p[1], p[3]) + p[5]

def p_parallelMoreFuncs(p):
    '''parallelMoreFuncs : parallelMoreFuncs exprfuncall SEP
                         | epsilon'''
    if (p[1] == None):
        p[0] = ()
    else:
...
def compile(source, optimizationLevel, debugHandle=None):
    ...
    if optimizationLevel > 0:
        ...
        try:
            ...
            msg = str(msg)
            opttree = ()
            sys.stderr.write("INTERNAL ERROR FROM OPTIMIZER: " \
                + msg + '\n')
...
def __link__(codearray, symtable, dumpfile):
    ...
    for i in xrange(0, len(codearray)):
        ...
        if (t == tuple and len(codearray[i]) == 1                   \
            ...
            message = "WARNING, DEPRECATED, non-string entry in program memory"
            message += " location: " + str(i) + ", value = " + str(codearray[i])
            sys.stdout.write(message + '\n')
            sys.stderr.write(message + '\n')
            codearray[i] = str(codearray[i])
    if (dumpfile):
