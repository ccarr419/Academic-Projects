# crunchlog.py, D. Parson, fall 2013
...
def stddev(valueList, average=None):
    ...
    result = math.sqrt(variance / float(len(valueList)))
    return result
...
if __name__ == '__main__':
    ...
    for tid in reportkeys:
        ...
        for state in statekeys:
            ...
            mix, max = minmax(threaddata[tid][state])
    ...
    diffset = (
        'MEAN_running', 'MEAN_ready', 'MEAN_waiting', 'MEAN_TURNAROUNDTIME',
        'MAX_running', 'MAX_ready', 'MAX_waiting', 'MAX_TURNAROUNDTIME',
        'MIN_running', 'MIN_ready', 'MIN_waiting', 'MIN_TURNAROUNDTIME'
      )
    iswarn = False
    for prop in diffset:
        old = oldstats[prop]
        ...
        if old == 0:
            old = 1
        if (float(diff) / float(old)) > 0.15 and diff >= 10:
