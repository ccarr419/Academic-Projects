/**
 *  stepvmTokenInline.c enhances stepvmTokenInC by eliminating the
 *  while(1) loop in run_stepvmInline() and replacing the break
 *  statements in that function with an invocation of macro
 *  DECODE_AND_DISPATCH. See Section 2.4.2 of the textbook.
 *  In tests on Linux machine mcgonagall, the in-line DECODE_AND_DISPATCH
 *  actually runs slower that using the loop and continue statement.
 *  The compiler does a good job of optimizing. Assembly language
 *  would likely show an improvement for in-line DECODE_AND_DISPATCH.
...
/* Initialize the data fields and arrays per docs in stepvmInline.h. */
void stepvmInline_constructor(struct stepvmInC *addressOfVM,
...
void dump_stepvmInline(struct stepvmInC *vm) {
    int i ;
...
#define DECODE_AND_DISPATCH {                                               \
        opcode = vm->codeMemory[vm->ip];                         \
        vm->ip += 1 ;   /* point to in-line data or next instruction */     \
        /* We cannot get rid of the switch statement in Token. */           \
        break ;                                                             \
}

void run_stepvmInline(struct stepvmInC *vm, int dumpOnTermination) {
    stepdata_t accum ;
    stepcode_t opcode ;
    int scanbuf ;
    vm->ip = 0 ;
        opcode = vm->codeMemory[vm->ip];
        vm->ip += 1 ; /* DECODE_AND_DISPATCH has a break statement! */
    while (1) {
        switch(opcode) {
            /** USER MODE INSTRUCTIONS, see stepvmInline.h **/
            case STEP_LADD :
...
int main(int argc, char **argv) {
    static char *USAGE =
    "stepvmInline DUMPONTERMINATION? TEST(e.g.BASIC) REPCOUNT\n";
    int isdump = 1 ;
    int i ;
    struct stepvmInC *vm = (struct stepvmInC *)
        malloc(sizeof(struct stepvmInC));
    stepvmInline_constructor(vm,NULL,0,NULL,0,NULL,0,NULL,0,1);
    if (argc < 4 || ((argc & 1) != 0)) {
