    ...
    if sys.argv[1] == 'f1' or sys.argv[1] == 'xf1':
        ...
        for vname in sys.argv[3:]:
            ...
            if miny < 0:
                ...
                miny = min(miny, v)
            xlbls[tid].append(vname.replace('rr_','').replace('_',''))
            csvhdr = csvhdr + "," + vname
        ...
        for fname in sys.argv[3:]:
            ...
            except ImportError, istr:
                sys.stderr.write("Skipping missing module " + fname + '\n')
                continue
            v = eval(vname)
            plotlist.append(v)
            maxy = max(maxy, v)
            if miny < 0:
                ...
                miny = min(miny, v)
            xlbls.append(fname.replace('count','').replace('_crunch','').replace('rr_','').replace('_','').replace('all','a').replace('swap','fifo').replace('min','mn').replace('max','mx'))
            csvhdr = csvhdr + "," + fname
            csvvalues = csvvalues + "," + str(v)
        # old plt.ylabel('m1 ' + sys.argv[2])
        # old if iszeroed:
            # old plotlist.append(0)
            # old xlbls.append('0')
        # old xs = range(len(xlbls))
        ...
        # old ax.xaxis.set_major_locator(MaxNLocator(integer=True))
        # old plt.plot(plotlist)
        # old fig.savefig(vname + '.png')
        # plt.show()
        # START STUFF ADDED FOR PAIRED BAR GRAPHS 9/24/2016
        # http://people.duke.edu/~ccc14/pcfb/numpympl/MatplotlibBarPlots.html
        ## necessary variables
        ind = np.arange(len(plotlist)) # # the x locations for the groups
        width = 0.35 # the width of the bars
        ## the bars
        rects1 = ax.bar(ind, plotlist, width,
            color='gray',yerr=None)
        # rects2 = ax.bar(ind+width, plotlist[1], width,
            # color='gray',yerr=None)
        ax.set_xlim(-width,len(ind)+width)
        ...
        ax.set_title("CSC343 2016 assignment 2: " + sys.argv[2])
        xTickMarks = xlbls
        # ax.set_xticks(ind+width)
