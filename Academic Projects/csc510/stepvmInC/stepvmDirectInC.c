 ...
 *  Edited Date:        09/20/2017
 *  Due Date:           09/28/2017
 *  Assignment:         #1
 *  File:               stepvmDirectInC.c
 ...
 *  Purpose:            The goals of this assignment are to understand various 
 *                      forms of threaded, emulated virtual machines (opcode 
 *                      instruction interpreters), where threads here refer to 
 *                      pointers to objects and code, not to threads of
 *                      execution as in parallel processing. This exact file
 *                      simulates a virtual machine through direct-threading.
**/
#include <stdio.h>
#include <assert.h>
#include "stepvmInC.h"
...
void run_stepvmInC(struct stepvmInC *vm, int dumpOnTermination) {
    stepdata_t accum ;
    int scanbuf ;
    static void* opCodeLabelTable[] = {
        &&LABEL_STEP_LADD, &&LABEL_STEP_LSUB, &&LABEL_STEP_LMULT,
        &&LABEL_STEP_LDIV, &&LABEL_STEP_LMOD, &&LABEL_STEP_LSHL,
        &&LABEL_STEP_LSHR, &&LABEL_STEP_LBITAND, &&LABEL_STEP_LBITOR,
        &&LABEL_STEP_LBITXOR, &&LABEL_STEP_LCOMPLEMENT, &&LABEL_STEP_LLOGAND,
        &&LABEL_STEP_LLOGOR, &&LABEL_STEP_LLOGNOT, &&LABEL_STEP_DUP,
        &&LABEL_STEP_DUP2, &&LABEL_STEP_OVER, &&LABEL_STEP_OVER2,
        &&LABEL_STEP_SWAP, &&LABEL_STEP_SWAP2, &&LABEL_STEP_LFETCH,
        &&LABEL_STEP_LSTORE, &&LABEL_STEP_GOTO, &&LABEL_STEP_GOTO0,
        &&LABEL_STEP_CALL, &&LABEL_STEP_RETURN, &&LABEL_STEP_SYSCALL,
        &&LABEL_STEP_CONST, &&LABEL_STEP_DROP, &&LABEL_STEP_DROP2,
        NULL, NULL, /* There is a gap in the opcode space here. */
        &&LABEL_STEP_SYSRETURN, &&LABEL_STEP_LINPUT, &&LABEL_STEP_LOUTPUT,
        &&LABEL_STEP_LDATAMEM, &&LABEL_STEP_LCODEMEM, &&LABEL_STEP_STARTUSER,
        &&LABEL_STEP_TERMINATE_VM
    };
    if (codePointers == NULL) {
        ...
        for (codeix = 0
            ...
            if (opcode >= 0 && opcode <= STEP_TERMINATE_VM) {
                codePointers[codeix] = opCodeLabelTable[opcode];
    ...
    vm->ip = 0 ;
    while (1) {
        void *jumpcode = codePointers[vm->ip];
#ifdef DEBUGVM
        fprintf(stderr,"DEBUG value of jumpcode = %lx, opcode = %d\n",
            128 (long)jumpcode, (int)(vm->codeMemory[vm->ip]));
#endif
