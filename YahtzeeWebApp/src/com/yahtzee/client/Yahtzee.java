...
public class Yahtzee implements EntryPoint {
            ...
            greetingService.start(new AsyncCallback<Integer>() {
                ...
                public void onSuccess(Integer result) {
                    if (result == 1 || result == 2) {
                        ...
                        RootPanel sc = RootPanel.get("ScoreContainer");
                        Element ss = Document.get().getElementById("Scoresheet");
                        TabPanel tp = (TabPanel) sc.getWidget(0);
    ...
    */
    public void onModuleLoad() {
        ...
        //Ask the user if they want to leave the game before closing web page
        Window.addWindowClosingHandler(new Window.ClosingHandler() {
            @Override
            public void onWindowClosing(Window.ClosingEvent event) {
                event.setMessage("Are you sure you want to quit a game in progress?");
            }
        });
        //Reset the server to allow new players on web page close
    ...
    */
    public void resetRound() {
        ...
        if(players > 1) {
            ...
            RootPanel.get("appFront").getElement().removeAttribute("hidden");
            e.setAttribute("class", "gamePlayerOpp");
            e.setInnerText("Other Player's Turn"); //Show its their opponents turn
